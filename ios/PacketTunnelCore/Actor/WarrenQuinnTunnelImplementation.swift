...
/// `MultiHopClient` → `NEPacketTunnelFlow`.
public final class WarrenQuinnTunnelImplementation: TunnelImplementation, @unchecked Sendable {
    ...
    private static func broadcastStats(_ status: WarrenTunnelStatus, into defaults: UserDefaults?) {
        ...
        // The FFI status struct carries no obfuscation field because the
        // HTTP/3 mimicry layer is unconditionally on (warren_tunnel_ffi.rs
        // hardcodes `use_warren_obfuscation: true`), so a connected tunnel
        // implies an obfuscated one. Mirror that into the App Group flag
    ...
    public func stopTunnel() async {
        ...
        // Clear the flag synchronously instead of relying on the next
        // 2 s stats tick: iOS may kill the extension right after stop,
        // which would strand a stale `true` in the App Group container
        // and leave the main app's obfuscation indicator lit forever.
        if let suiteName = appGroupSuiteName, let defaults = UserDefaults(suiteName: suiteName) {
            defaults.set(false, forKey: WarrenAppGroupKey.obfuscationActive.rawValue)
            // Same stale-flag rationale for the NAT-PMP surface: the exit
    ...
    private static func broadcastEvent(_ event: WarrenTunnelEvent, into defaults: UserDefaults?) {
        ...
        case .natPmpRenewed(let externalPort):
            // A renewal carries no lifetime on the Swift event surface;
            // keep the previously granted lifetime and just refresh the
            // grant timestamp so the countdown restarts.
            defaults.set(Int(externalPort), forKey: WarrenAppGroupKey.natPmpExternalPort.rawValue)
            defaults.set("open", forKey: WarrenAppGroupKey.natPmpStatus.rawValue)
            defaults.set(Date(), forKey: WarrenAppGroupKey.natPmpMappedAt.rawValue)
        case .natPmpFailed:
    ...
    /// Remove every NAT-PMP surface key. Called on disconnect and on
    /// tunnel stop so the main app never renders a mapping that no
    /// longer exists on the exit.
    private static func clearNatPmpKeys(in defaults: UserDefaults) {
        defaults.removeObject(forKey: WarrenAppGroupKey.natPmpExternalPort.rawValue)
