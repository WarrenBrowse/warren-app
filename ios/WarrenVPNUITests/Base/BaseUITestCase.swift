...
class BaseUITestCase: XCTestCase {
    ...
    /// Suites run with unfunded throwaway wallets on the sim, so the app
    /// assumes an active subscription by default; out-of-time gate tests
    /// opt out to exercise the real expired routing.
    class var assumeSubscribed: Bool { true }
    ...
    override func setUp() async throws {
        ...
        if Self.assumeSubscribed {
            app.launchEnvironment["WARREN_QA_ASSUME_SUBSCRIBED"] = "1"
        }
        app.launch()
