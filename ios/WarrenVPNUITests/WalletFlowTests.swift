...
//  (`WarrenWalletLoginCoordinator`) when TOS is agreed AND no wallet
//  exists in the Keychain (see `nextWarrenRoutes`; login now precedes
//  the onboarding wizard, desktop parity). `AppResetManager.resetKeychain()`
//  wipes the wallet Keychain when the `.settings` reset key is set, so a
//  `.forceLoggedOut` + reset-all launch guarantees the no-wallet half.
//  Onboarding-complete is pre-seeded via `appPreferencesSeed` so that
//  AFTER the wallet is provisioned these tests land directly on `.main`
//  instead of walking the wizard (covered by `OnboardingWizardTests`).
//  TOS is auto-agreed by the base `setUp`.
//
