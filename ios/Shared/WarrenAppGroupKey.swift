...
public enum WarrenAppGroupKey: String, CaseIterable {
    ...
    /// `String`. Live NAT-PMP mapping status: "open" after a grant,
    /// "failed" after a refused/failed request. Absent while no request
    /// has resolved (the main app renders that as "requesting" when the
    /// setting is on and the tunnel is up). Cleared on disconnect.
    case natPmpStatus = "WarrenTunnel.natPmpStatus"

    /// `Date`. Time the last NAT-PMP grant (Mapped/Renewed) was
    /// recorded. Combined with [`natPmpLifetimeSeconds`] to render the
    /// renew countdown (the tunnel renews at half-lifetime).
    case natPmpMappedAt = "WarrenTunnel.natPmpMappedAt"

    /// `Int`. Lifetime in seconds the exit granted on the last NAT-PMP
