...
class SelectLocationViewModelImpl: SelectLocationViewModel {
    ...
    func onFilterTapped(_ filter: SelectLocationFilter) {
        ...
        case .owned, .rented, .provider:
            // Ownership/provider relay filtering has no meaning on the Warren
            // network, so its filter surface was removed (desktop parity).
            // These chips can no longer be set from anywhere, so the tap is a
            // no-op kept only to satisfy the exhaustive switch.
            break
        case .daita:
