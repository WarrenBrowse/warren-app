    ...
    func configureCell(_ cell: UITableViewCell, item: CustomDNSDataSource.Item, indexPath: IndexPath) {
        ...
        case .dnsServerInfo:
            ...
            cell.titleLabel.attributedText = viewModel.customDNSPrecondition.attributedLocalizedDescription(
                isEditing: isEditing,
                preferredFont: .warrenTiny
            )

        case .allowExternalDns:
            guard let cell = cell as? SettingsSwitchCell else { return }

            cell.titleLabel.text = NSLocalizedString("Allow external DNS resolvers", comment: "")
            cell.setAccessibilityIdentifier(item.accessibilityIdentifier)
            cell.setOn(viewModel.allowExternalDns, animated: false)
            cell.infoButtonHandler = { [weak self] in
                self?.delegate?.showInfo(for: .allowExternalDns)
            }
            cell.action = { [weak self] isOn in
                self?.delegate?.didChangeState(for: .allowExternalDns, isOn: isOn)
            }
            cell.backgroundView?.backgroundColor = UIColor.Cell.Background.normal
        ...
        case .allowExternalDnsInfo:
            ...
            // Permanent privacy-warning footer, mirroring the desktop info
            // modal's second paragraph.
            cell.titleLabel.attributedText = NSAttributedString(
                string: NSLocalizedString(
                    "The queries still travel through the VPN tunnel, but the resolver you choose "
                        + "will see them. Leave this off unless you are testing remote DNS resolution.",
                    comment: ""
                ),
                attributes: [.font: UIFont.warrenTiny]
            )
        }
    }
