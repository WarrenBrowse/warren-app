...
>, UITableViewDelegate
{
    ...
    enum Item: Hashable {
        ...
        var accessibilityIdentifier: AccessibilityIdentifier {
            ...
            case .allowExternalDns:
                return .allowExternalDns
            case .allowExternalDnsInfo:
                return .allowExternalDnsInfo
            }
        }
        ...
        var reuseIdentifier: CellReuseIdentifiers {
            ...
            case .dnsServer:
                return .dnsServer
            case .dnsServerInfo, .allowExternalDnsInfo:
                return .dnsServerInfo
            default:
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        ...
        // Headerless spacer separating the toggle from the custom DNS list
        // above (matches the inter-section footer spacing used elsewhere).
        case .allowExternalDns:
            return 16
    ...
    private func updateSnapshot(animated: Bool = false, completion: (() -> Void)? = nil) {
        ...
        // The toggle mirrors desktop's placement, directly below the custom
        // DNS block. The info row doubles as the permanent privacy-warning
        // footer.
        newSnapshot.appendItems([.allowExternalDns, .allowExternalDnsInfo], toSection: .allowExternalDns)
    ...
    private func setAllowExternalDns(_ isEnabled: Bool) {
        viewModel.setAllowExternalDns(isEnabled)
        // Not part of `DNSSettings`, so it bypasses the edit-mode staging
        // used by the custom DNS list and applies immediately, like the
        // desktop toggle.
        delegate?.didChangeAllowExternalDns(isEnabled)
    }
