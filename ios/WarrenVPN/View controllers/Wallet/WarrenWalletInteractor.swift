...
public final class WarrenWalletInteractor: @unchecked Sendable {
    ...
    /// Loads the wallet mnemonic from the Keychain WITHOUT an
    /// authentication prompt, for the first-run onboarding backup
    /// reminder only. Acceptable there because the phrase was already
    /// on screen seconds earlier in the same session (shown at creation
    /// or typed at restore), and the desktop wizard behaves the same.
    /// Every later reveal (Settings) must keep using
    /// `loadMnemonicWithAuth(reason:completion:)`.
    public func loadMnemonicForOnboardingBackup(
        completion: @escaping @Sendable @MainActor (Result<String, WarrenWalletInteractorError>) -> Void
    ) {
        queue.async {
            do {
    ...
    public func fetchSubscriptionExpiry(
        ...
        #if DEBUG
            // Sim UI tests drive connect flows with unfunded throwaway
            // wallets; this override keeps them past the out-of-time gate.
            if ProcessInfo.processInfo.environment["WARREN_QA_ASSUME_SUBSCRIBED"] == "1" {
                queue.async { completion(.success(.distantFuture)) }
                return
            }
        #endif
        queue.async { [weak self] in
