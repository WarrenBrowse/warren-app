...
class AlertViewController: UIViewController {
    ...
    /// Buttons that stay disabled until the checkbox is ticked.
    private var checkboxGatedButtons = [AppButton]()
    ...
    func add(action: AlertAction) {
        addAction(
            ...
            isGatedByCheckbox: action.isGatedByCheckbox,
    ...
    private func setContent() {
        ...
        presentation.checkbox.flatMap { addCheckbox($0) }
    ...
    private func addAction(
        ...
        if isGatedByCheckbox {
            button.isEnabled = false
            checkboxGatedButtons.append(button)
        }
    ...
    private func addCheckbox(_ checkbox: AlertCheckbox) {
        let box = UIImageView(image: UIImage(systemName: "square"))
        box.tintColor = .white
        ...
        row.alignment = .top
        row.spacing = 8
        row.setAccessibilityIdentifier(checkbox.accessibilityId ?? .alertContainerView)
        row.isAccessibilityElement = true
        row.accessibilityTraits = .button
        row.accessibilityLabel = checkbox.title
        // The whole row is the hit target so the long acknowledgement text is
        // tappable, not just the small box.
        row.isUserInteractionEnabled = true
        row.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(didTapCheckboxRow)))

        contentView.addArrangedSubview(row)
    }

    @objc private func didTapCheckboxRow() {
        isCheckboxChecked.toggle()
        checkboxImageView?.image = UIImage(
            systemName: isCheckboxChecked ? "checkmark.square.fill" : "square"
        )
        checkboxGatedButtons.forEach { $0.isEnabled = isCheckboxChecked }
    }
