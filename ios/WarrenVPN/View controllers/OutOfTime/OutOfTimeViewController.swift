...
class OutOfTimeViewController: UIViewController, RootContainment {
    ...
    override func viewDidLoad() {
        ...
        contentView.webCheckoutButton.addTarget(
            self,
            action: #selector(requestWebCheckout),
            for: .touchUpInside
        )
        contentView.redeemVoucherButton.addTarget(
            self,
            action: #selector(requestRedeemVoucher),
            for: .touchUpInside
        )
    ...
    private func applyViewState() {
        ...
        contentView.enablePurchaseButtons(!tunnelState.isSecured)
        ...
        if tunnelState.isSecured {
            ...
            contentView.setBodyLabelText(
                [
                    baseMessage,
                    NSLocalizedString(
                        "Tap **Add time** to buy more time directly, or pay by card "
                            + "on the web and redeem the voucher you receive.",
                        comment: ""
                    ),
                ].joined()
            )
    ...
    @objc private func requestWebCheckout() {
        delegate?.didRequestOpenWebCheckout()
    }

    @objc private func requestRedeemVoucher() {
        delegate?.didRequestRedeemVoucher()
    }
