...
public enum TunnelSettingsUpdate: Sendable {
    ...
    case natPmp(WarrenNatPmpSettings)
    case allowExternalDns(WarrenAllowExternalDnsSettings)
}
...
extension TunnelSettingsUpdate {
    public func apply(to settings: inout LatestTunnelSettings) {
        ...
        case let .natPmp(newNatPmpSettings):
            settings.natPmp = newNatPmpSettings
        case let .allowExternalDns(newAllowExternalDnsSettings):
            settings.allowExternalDns = newAllowExternalDnsSettings
        }
    }
    ...
    public var subjectName: String {
        ...
        case .natPmp: "NAT-PMP port forwarding"
        case .allowExternalDns: "allow external DNS resolvers"
        }
    }
}
