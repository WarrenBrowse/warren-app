...
public struct RelayConstraints: Codable, Equatable, CustomDebugStringConvertible, @unchecked Sendable {
    ...
    // Factory default is `.any` (automatic pick): the upstream Mullvad
    // default pinned `se`, and the Warren fleet serves no Swedish exit,
    // so every fresh install was blocked with relayConstraintNotMatching
    // once the fetched directory replaced the prebundled list.
    public init(
        entryLocations: RelayConstraint<UserSelectedRelays> = .any,
        exitLocations: RelayConstraint<UserSelectedRelays> = .any,
