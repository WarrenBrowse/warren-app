...
#
# Used by the macOS and Windows release jobs. On Windows it also absorbs a
# transient linker failure (LNK1181 "cannot open input file
# windows.0.52.0.lib"): the windows-rs import lib exists in the shared cargo
# registry with the correct /LIBPATH, but a concurrent runner's build or the
# persistent target can leave it briefly unopenable at link time. A clean-target
# retry lands on a fresh state and succeeds; a genuine break fails both attempts.
#
# Why (macOS): the macOS release build is universal. build.sh compiles the native
