...
class WarrenTunInterfacePlanTest {
    ...
    @Test
    fun `allow lan keeps the exit gateway tunnelled whatever resolver is configured`() {
        // 10.66.0.1 is not only the default resolver, it is the tunnel gateway
        // the in-tunnel egress probe queries. Once the user picks another
        // resolver it stops being a DNS server, and 10/8 is excluded under
        // allow-LAN: without an explicit route the probe would answer "the exit
        // forwards nothing" on a perfectly healthy tunnel and tear it down.
        val blocked = planTunInterface(
            config(
                allowLan = true,
                dns = WarrenTunnelConfig.DnsConfig(blockAds = true),
            )
        )
        assertTrue(
            blocked.coversV4(WarrenTunDefaults.EXIT_DNS_RESOLVER),
            "the exit gateway must stay tunnelled behind a content-blocking resolver"
        )
        val custom = planTunInterface(
            config(
                allowLan = true,
                dns = WarrenTunnelConfig.DnsConfig(
                    state = WarrenTunnelConfig.DnsConfig.STATE_CUSTOM,
        ...
        assertTrue(
            custom.coversV4(WarrenTunDefaults.EXIT_DNS_RESOLVER),
            "the exit gateway must stay tunnelled behind a custom resolver"
        )
    }
