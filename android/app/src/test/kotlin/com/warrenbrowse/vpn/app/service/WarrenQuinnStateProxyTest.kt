...
class WarrenQuinnStateProxyTest {
    ...
    @Test
    fun `a teardown in flight is projected as its own state`() = runTest {
        val proxy = WarrenQuinnStateProxy()
        proxy.update(WarrenTunnelState.Disconnecting())
        assertEquals("Disconnecting...", proxy.state.value)
        assertEquals(WarrenConnectedInfo.Disconnecting(), proxy.connectedInfo.value)
    }

    @Test
    fun `a dial in flight carries its config to the connection card`() = runTest {
        val proxy = WarrenQuinnStateProxy()
        proxy.update(
            WarrenTunnelState.Connecting(
                exitEndpointHost = "185.65.135.10:443",
                entryEndpointHost = "146.70.1.2:443",
                multiHop = true,
                daita = true,
            )
        )
        val info = proxy.connectedInfo.value as WarrenConnectedInfo.Connecting
        assertEquals("185.65.135.10:443", info.exitEndpointHost)
        assertEquals("146.70.1.2:443", info.entryEndpointHost)
        assertTrue(info.multiHop)
        assertTrue(info.daita)
    }
