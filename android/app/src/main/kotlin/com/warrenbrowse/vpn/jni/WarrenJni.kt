...
object WarrenJni {
    ...
    /**
     * Waits up to [timeoutMs] for the session [disconnectTunnel] wound down to
     * actually finish. `1` = gone (or nothing to wait for), `0` = timed out.
     *
     * Dialling the next session before this returns registers it on the exit
     * while the previous one still holds the sticky inner IP, and the exit then
     * has two live claims on one downlink route.
     */
    external fun awaitTunnelClosed(timeoutMs: Int): Int

    /**
     * Report an underlying-network handover (Wi-Fi to cellular and back) to
     * the native migration watchdog. It rebinds the live QUIC endpoint onto a
     * fresh `VpnService.protect`-ed socket and revalidates the path in about
     * one RTT, so the session survives the handover instead of being torn down
     * for a full re-handshake.
     *
     * Never blocks and never throws: it hands one event to a channel. When the
     * path cannot be recovered the watchdog ends the session, so
     * [getTunnelStatus] reports disconnected and the Kotlin fail-closed policy
     * runs the handover fallback. A no-op when no tunnel is running.
     */
     ...
     * "payments_enabled":...}` or `{"ok":false}` on any failure. Blocks
     * on a network fetch; call off the main thread.
     */
