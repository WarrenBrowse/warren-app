...
fun planTunInterface(
    ...
    //
    // The exit gateway is re-added even when it is not the configured resolver:
    // it is what the in-tunnel egress probe queries, so leaving it in the
    // excluded 10/8 would make the probe report "the exit forwards nothing" on
    // a healthy tunnel and tear it down.
    val ipv4Routes =
        if (config.allowLan) {
            ipv4RoutesExcluding(LAN_EXCLUDED_IPV4) +
                (dnsServers.filter { !it.contains(':') } + WarrenTunDefaults.EXIT_DNS_RESOLVER)
                    .distinct()
