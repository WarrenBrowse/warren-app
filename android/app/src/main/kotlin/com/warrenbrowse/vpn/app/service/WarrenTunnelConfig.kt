...
data class WarrenTunnelConfig(
    ...
    // Multi-hop topology, mirroring the iOS FFI `multihop_two_hop`. `true`
    // (the default, preserving today's Android behavior) selects a 2-hop
    // circuit: a distinct entry relay in front of the exit. `false` selects
    // the single-hop 1-hop circuit, where the exit node is dialed as its own
    // entry relay on the unified `:443` dispatcher (one node sees both the
    // user IP and the destination). Either way the tunnel rides the multi-hop
    // wire (the fleet speaks only that), so `entry_hop` + `multihopDirectoryRaw`
    // are always sent. Older native libs ignore this unknown field and stay
    // 2-hop, matching the Rust `None` default.
    @SerialName("multihop_two_hop") val multihopTwoHop: Boolean = true,
    ...
    // Client-side bandwidth ceiling in bits per second, enforced by the
    // Rust datapath on both directions independently. 0 = unlimited.
    ...
    // `relay_endpoint` (the entry endpoint comes from the directory). `entry_hop`
    // is always sent, even for a 1-hop circuit: warren-jni requires it and fails
    // closed without it (the production fleet speaks only the multi-hop wire).
    @Serializable
    data class EntryHop(
