...
// A release ships the ART baseline profile in src/main/baseline-prof.txt,
// generated by test/baselineprofile on Warren's flow (docs/BuildInstructions.md).
// AGP packages whatever it finds there and nothing when the file is missing,
// which is how a regeneration that landed no file shipped profile-less
// releases once: cold start and the first Connect frames ran interpreted until
// the next background dexopt. The check runs before the profile is merged
// into any release-shaped variant; the non-minified ones the generator itself
// builds are exempt, since the first generation starts with no file.
val checkBaselineProfile by tasks.registering {
    description = "Refuses a release build without the generated ART baseline profile."
    val profile = layout.projectDirectory.file("src/main/baseline-prof.txt").asFile
    doLast {
        check(profile.isFile) {
            "${profile.path} is missing: generate it with " +
                ":app:generateProdReleaseBaselineProfile (docs/BuildInstructions.md)"
        }
        check(profile.useLines { lines -> lines.any { it.contains("Lcom/warrenbrowse/") } }) {
            "${profile.path} names no Warren class: it was generated against another app"
        }
    }
}

tasks
    .matching { it.name.matches(Regex("merge(Prod|Beta|Staging)(Benchmark)?ReleaseArtProfile")) }
    .configureEach { dependsOn(checkBaselineProfile) }
