...
/**
 * Scoped to a NavBackStackEntry that displays a staged phrase (the create-time
 * [com.warrenbrowse.vpn.feature.login.api.WarrenWalletBackupNavKey] and the
 * later read-back [com.warrenbrowse.vpn.feature.login.api.WarrenKeysNavKey]):
 * consumes the staged [Mnemonic] from [MnemonicCache] once at construction
 * time, holds it for the lifetime of the back-stack entry, and closes it
 * (zeroing its [CharArray]) when [onCleared] fires.
 *
 ...
 * toggle, ...) destroys and recomposes the Activity, re-runs the
