...
class NatPmpStatusLabelTest {
    ...
    private val context: Context = mockk {
        ...
        every { getString(R.string.tunnel_natpmp_status_waiting_tunnel) } returns
    ...
    fun `idle and empty render the idle label`() {
        assertEquals("Status: idle (no active mapping)", natPmpStatusLabel(context, tunnelConnected = true, json = """{"state":"idle"}"""))
        assertEquals("Status: idle (no active mapping)", natPmpStatusLabel(context, tunnelConnected = true, json = "{}"))
    }

    @Test
    fun `with no tunnel the fallback names the cause instead of saying idle`() {
        // "idle" leaves the user unable to tell a broken feature from one that
        // is simply waiting for a tunnel; only a live tunnel can be idle.
        assertEquals(
            "Status: waiting for an active tunnel connection to set up the port mappings.",
            natPmpStatusLabel(context, tunnelConnected = false, json = """{"state":"idle"}"""),
        )
    }

    @Test
    fun `a live mapping status is rendered whatever the tunnel flag says`() {
        assertEquals(
            "Status: mapped - external port 51820",
            natPmpStatusLabel(
                context,
                tunnelConnected = false,
                json = """{"state":"mapped","external_port":51820}""",
            ),
        )
    }
    ...
    fun `requesting renders progress`() {
        assertEquals("Status: requesting a port…", natPmpStatusLabel(context, tunnelConnected = true, json = """{"state":"requesting"}"""))
    }

    @Test
    fun `mapped renders port and lifetime`() {
        val json = """{"state":"mapped","external_port":51820,"lifetime_secs":3600}"""
        assertEquals("Status: mapped - external port 51820 (lifetime 3600s)", natPmpStatusLabel(context, tunnelConnected = true, json = json))
    }
    ...
    fun `rate limited renders retry countdown`() {
        assertEquals(
            ...
            natPmpStatusLabel(context, tunnelConnected = true, json = """{"state":"rate_limited","retry_after_secs":30}"""),
        )
    }
    ...
    fun `failed renders the reason category`() {
        assertEquals(
            ...
            natPmpStatusLabel(context, tunnelConnected = true, json = """{"state":"failed","reason":"Unreachable"}"""),
        )
    }
    ...
    fun `failed with a port conflict renders a friendly actionable message`() {
        ...
        assertEquals(
            ...
            natPmpStatusLabel(context, tunnelConnected = true, json = """{"state":"failed","reason":"SuggestedPortInUse"}"""),
        )
    }
