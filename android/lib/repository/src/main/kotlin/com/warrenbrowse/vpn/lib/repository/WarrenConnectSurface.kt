...
sealed interface WarrenConnectedInfo {
    ...
    /**
     * A dial in flight, carrying the config being dialled. The card names the
     * exit and raises its chips from the first frame instead of guessing a
     * relay and correcting itself the moment the tunnel comes up.
     *
     * The hosts are null until a config exists (a dispatch the engine has not
     * picked up yet).
     */
    ...
    /**
     * Teardown in flight. [reconnecting] marks a teardown that is already on
     * its way back up (a settings change or a location switch re-dialling the
     * tunnel), which the card presents as a connection in progress rather than
     * as a disconnect.
     */
 ...
 *
 * [state] is a legacy `String` projection, hard-coded English carrying the
 * engine's failure reason verbatim. Its only remaining consumer is the quick
 * settings tile, which prefix-matches it into a tile state and never shows it.
 * Never render it: a user-visible surface takes [connectedInfo], the lossless
 * typed projection, and maps it to string resources.
 */
 ...
 * adapter, which reads `WarrenJni.getNatPmpStatus()` on every status wake.
 * Consumers receive a raw JSON `String` projection (parsed UI-side).
 * `{"state":"idle"}` when port forwarding is off or no mapping is active.
 */
...
/**
 * Lib-side surface for the engine's datapath verdict. `true` while the tunnel
 * is up and carrying nothing (the goodput prober's wedge verdict), which the
 * UI must not render as protection.
 */
...
interface WarrenRelayProvider {
    /**
     * The cached catalogue as a stream. Consumers that derive state from the
     * relay list observe this so they recompute when a [refresh] lands,
     * instead of latching the empty snapshot they saw on a cold cache.
     */
    val catalogue: StateFlow<List<WarrenRelaySummary>>

    /**
     * Cached snapshot of the available relays. Never performs I/O, so it is
     * safe to read during composition. An empty list means the catalogue has
     * not been fetched yet, so pair it with [refresh].
     */
