 ...
 * (`WarrenJni.importMnemonic`); the counter is UI feedback only.
 *
 * The field is controlled: [phrase] is the single source of truth and
 * [onPhraseChange] hands back the text verbatim, so what the caller enables its
 * CTA on is always what is on screen. Canonicalize with [normalizeMnemonic]
 * before handing the phrase to the daemon, never on the way in: trimming while
 * the user types would swallow the space between two words.
 */
@Composable
fun MnemonicInput(
    phrase: String,
    ...
    val count = remember(phrase) { countMnemonicWords(phrase) }
    ...
    Column(
        ...
        OutlinedTextField(
            ...
            modifier = Modifier
                .fillMaxWidth()
                // Keep the typed phrase out of the accessibility tree and
                // sensitivity-honoring screenshots.
