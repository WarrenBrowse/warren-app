package com.warrenbrowse.vpn.test.e2e.constant
...
fun Bundle.isBillingEnabled(): Boolean =
    InstrumentationRegistry.getArguments()
        .getString("warren.test.e2e.config.billing.enable", "false")
        .toBoolean()

fun Bundle.isRaasEnabled(): Boolean =
    InstrumentationRegistry.getArguments()
        .getRequiredArgument("warren.test.e2e.config.raas.enable")
        .toBoolean()

fun Bundle.isHighlyRateLimitedTestsEnabled(): Boolean =
    InstrumentationRegistry.getArguments()
        .getRequiredArgument("warren.test.e2e.config.runHighlyRateLimitedTests")
        .toBoolean()

fun Bundle.getRaasHost() =
    InstrumentationRegistry.getArguments().getRequiredArgument("warren.test.e2e.config.raas.host")

fun Bundle.getTrafficGeneratorHost(): String =
    InstrumentationRegistry.getArguments()
        .getRequiredArgument("warren.test.e2e.config.raas.trafficGenerator.target.host")

fun Bundle.getTrafficGeneratorPort(): Int =
    InstrumentationRegistry.getArguments()
        .getRequiredArgument("warren.test.e2e.config.raas.trafficGenerator.target.port")
        .toInt()
