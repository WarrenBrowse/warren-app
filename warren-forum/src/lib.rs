...
//! in-app bug report (`POST /v1/forum/report`) and the forum page's
//! attach-logs upload (`POST /v1/forum/attach-logs`), the shared, host-tested
...
fn has_clock_skew_token(body: &[u8]) -> bool {
    body_carries(body, CLOCK_SKEW_BODY_TOKEN)
}

/// True iff `body` carries `token` as a substring: the frozen machine tokens
/// connect answers with are short enough that no JSON parse is needed.
...
/// The topic id of a report still being composed: the forum binds the logs
/// to the topic after creation, so the app parks them under the session.
pub const PRE_TOPIC_ID: u64 = 0;
...
pub const MAX_FORUM_TOPIC_ID: u64 = (1 << 53) - 1;

/// A topic id as a deep link, or the consent prompt's field, spells it:
/// decimal digits only (so no sign), within [`MAX_FORUM_TOPIC_ID`]; `None`
/// otherwise. [`PRE_TOPIC_ID`] is the pre-topic variant.
#[must_use]
pub fn parse_topic_id(text: &str) -> Option<u64> {
    if text.is_empty() || !text.bytes().all(|b| b.is_ascii_digit()) {
        return None;
    }
    let value: u64 = text.parse().ok()?;
    (value <= MAX_FORUM_TOPIC_ID).then_some(value)
}

/// The canonical body of the attach-logs upload: the session id, the topic
/// ([`PRE_TOPIC_ID`] for a pre-topic session) and the gzipped redacted report
/// as `log_gz_b64`. Serialised by the shared report-body builder, so the size
/// cap and the base64 alphabet are the ones every other upload uses. The
/// provider reads the object with serde; its own tests send this key order.
///
/// # Errors
///
...
/// `host` with `signing_key`, stamped with the corrected `timestamp`. The
/// mirror of the desktop daemon's `sign_forum_attach_logs`: the body is
...
/// is malformed, the log is empty or the RNG is unusable;
/// [`ForumRequestError::LogTooLarge`] over [`MAX_LOG_GZ_BYTES`].
...
/// the exchange itself plus 10 s per MiB of body. A transport's default of
/// 15 s is sized for a few hundred bytes, and a report with a few MiB of
/// logs on a slow mobile uplink (the network a report is filed from) died in
/// it as a generic transport failure after the data was spent. 10 s per MiB
/// is a floor of about 0.8 Mbit/s; a no-log report keeps roughly the mint's
/// own bound, a report at the log cap gets three minutes.
#[must_use]
pub fn upload_deadline(body_len: usize) -> std::time::Duration {
    const MIB: usize = 1024 * 1024;
    let mib = u64::try_from(body_len.div_ceil(MIB)).unwrap_or(u64::MAX);
    std::time::Duration::from_secs(20u64.saturating_add(10u64.saturating_mul(mib)))
}
...
/// Map the provider's answer to the attach outcome.
#[must_use]
pub fn attach_outcome_for_response(status: u16, body: &[u8]) -> ForumAttachOutcome {
    match status {
        ...
        401 if has_clock_skew_token(body) => ForumAttachOutcome::ClockSkew,
        500..=599 => ForumAttachOutcome::ServerError,
        other => ForumAttachOutcome::Failed(FailReason::Http(other)),
    }
}

/// The JSON envelope returned across the FFI for an attach outcome, in the
/// shape of [`envelope`] so the platform decoders are the login's. Never
/// carries any request context.
#[must_use]
pub fn attach_envelope(outcome: &ForumAttachOutcome) -> String {
    match outcome {
        ...
        ForumAttachOutcome::Failed(reason) => failed_envelope(*reason),
    }
}
...
mod tests {
    ...
    fn the_attach_body_pins_the_wire_contract() {
        ...
        assert_eq!(
            attach_body(SID, PRE_TOPIC_ID, b"hello").expect("pre-topic builds"),
            format!(r#"{{"log_gz_b64":"aGVsbG8=","sid":"{SID}","topic_id":0}}"#).into_bytes()
        );
    }

    #[test]
    fn the_attach_body_refuses_a_bad_sid_an_empty_log_and_one_over_the_broker_cap() {
        assert_eq!(
            attach_body("NOTHEX", 42, b"x"),
            Err(ForumRequestError::Invalid)
        );
    ...
    fn build_signed_attach_request_targets_the_attach_route_at_the_given_stamp() {
        ...
        .expect("a valid key + link must build a request");
        assert_eq!(
            req.url,
    ...
    fn build_signed_attach_request_refuses_what_the_login_builder_refuses() {
        assert_eq!(
            build_signed_attach_request(&signing_key(), SID, "evil.example.com", 42, b"gz", 1),
            Err(ForumRequestError::Invalid)
        );
        assert_eq!(
    ...
    fn the_attach_session_urls_follow_the_allowlist() {
        assert_eq!(
            build_attach_status_url(SID, "connect.warrenbrowse.com").as_deref(),
            Some(
                "https://connect.warrenbrowse.com/v1/attach/0123456789abcdef0123456789abcdef/status"
            )
        );
        assert_eq!(
            build_attach_cancel_url(SID, "connect.warrenbrowse.com").as_deref(),
            Some(
        ...
        assert_eq!(
            build_attach_cancel_url("NOTHEX", "connect.warrenbrowse.com"),
            None
        );
    }
    ...
    #[test]
    fn a_topic_id_is_decimal_digits_within_the_safe_integer() {
        // The fixture's rule: digits only (so no sign), at most 2^53 - 1
        // because the desktop carries it through a JavaScript number, and 0
        // is the pre-topic variant.
        assert_eq!(parse_topic_id("42"), Some(42));
        assert_eq!(parse_topic_id("0"), Some(PRE_TOPIC_ID));
        assert_eq!(parse_topic_id("9007199254740991"), Some(MAX_FORUM_TOPIC_ID));
        assert_eq!(parse_topic_id("9007199254740993"), None);
        assert_eq!(parse_topic_id("-1"), None);
        assert_eq!(parse_topic_id("abc"), None);
        assert_eq!(parse_topic_id(""), None);
        assert_eq!(parse_topic_id(" 42"), None);
    }
