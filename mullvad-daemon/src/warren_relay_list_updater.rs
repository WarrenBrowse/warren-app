...
pub fn write_cache_atomic(cache_path: &Path, raw: &str) -> Result<(), Error> {
    ...
    std::fs::write(&tmp, raw).map_err(|e| Error::Io(tmp.display().to_string(), e))?;
    std::fs::rename(&tmp, cache_path).map_err(|e| {
        let _ = std::fs::remove_file(&tmp);
        Error::Io(cache_path.display().to_string(), e)
    })?;
    Ok(())
}
...
impl WarrenRelayListUpdater {
    ...
    async fn fetch_roster_once(
        ...
        if status == 304 || status == 404 {
            return Ok(RosterFetch::NotModifiedOrAbsent);
        }
        if status != 200 {
    ...
    async fn fetch_once(
        ...
        let new_etag = resp
            ...
            .or(etag);
        if status == 304 {
            return decide_outcome(304, "", new_etag, pin.as_deref());
        }
        if status != 200 {
            return Err(Error::Status(status));
        }
        let body = resp.text().await?;
        decide_outcome(200, &body, new_etag, pin.as_deref())
    }
...
mod tests {
    ...
    fn write_cache_atomic_overwrites_previous_good_cache() {
        let dir = isolated_tempdir();
        let path = dir.join(WARREN_RELAYS_FILENAME);
        write_cache_atomic(&path, "v1").unwrap();
        write_cache_atomic(&path, "v2").unwrap();
