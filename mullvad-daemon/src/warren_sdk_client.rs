...
/// Delegates to a shared `Arc<ReqwestTransport>` so every `WarrenApiClient`
/// rebuilt by [`SharedWarrenApiClient`] reuses the same underlying
/// `reqwest::Client` connection pool instead of paying a fresh TLS/DNS
/// warmup on every call.
#[derive(Clone)]
struct SharedTransport(Arc<ReqwestTransport>);
...
impl SharedWarrenApiClient {
    ...
    pub fn new(api_base: String, seed: SharedWarrenSeed) -> Self {
        Self {
            api_base,
            seed,
            transport: SharedTransport(Arc::new(ReqwestTransport::new())),
        }
    }
    ...
    fn client(&self) -> WarrenApiClient<SharedTransport> {
        WarrenApiClient::new(
            self.api_base.clone(),
