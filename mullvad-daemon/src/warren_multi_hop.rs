...
use warrenguard_multihop::{
    ExitPkiError, RelayPkiError, verify_exit_descriptor, verify_exit_descriptor_pq,
...
pub fn load_from_settings_dir(settings_dir: &Path) -> Result<Option<MultiHopConfig>, Error> {
    ...
    // Accept a PQ-signed exit descriptor (ML-KEM key bound under the operational
    // signature) as well as a classical one, so a PQ-publishing exit is not
    // rejected. Neither verifier accepting still fails closed. No exit publishes
    // a PQ descriptor yet, so this resolves to the classical verifier as before.
    if verify_exit_descriptor_pq(&operational_pubkey, &parsed.exit).is_err() {
...
mod tests {
  ...
  "exit": {{
    ...
    fn pq_signed_exit_file(op: &SigningKey) -> String {
        let relay_id = [0x11; 16];
        let relay_pubkey = [0x22; 32];
        let relay_payload = relay_descriptor_signing_payload(&relay_id, &relay_pubkey);
        let relay_sig_bytes = op.sign(&relay_payload).to_bytes();

        let exit_id = ExitId::from_bytes([0x33; 16]);
        let exit_x25519 = [0x44; 32];
        // Deterministic bytes of the exact ML-KEM-768 encapsulation-key length:
        // the loader checks only the operational signature over the key, not its
        // internal structure.
        let mlkem_ek: Vec<u8> = (0..MLKEM768_ENCAPS_KEY_LEN)
            .map(|i| (i % 251) as u8)
            .collect();
        let exit_payload =
            exit_descriptor_signing_payload_pq(exit_id, &exit_x25519, false, &mlkem_ek);
        let exit_sig_bytes = op.sign(&exit_payload).to_bytes();
        ...
        let exit_x_hex = hex::encode(exit_x25519);
        let exit_sig_hex = hex::encode(exit_sig_bytes);
        let mlkem_hex = hex::encode(&mlkem_ek);
  ...
  "exit": {{
    ...
    "exit_ed25519_pubkey": "{exit_ed_hex}",
    "exit_x25519_multihop_pubkey": "{exit_x_hex}",
    "exit_mlkem768_pubkey": "{mlkem_hex}",
    "endpoint": "198.51.100.20:443",
    "signature": "{exit_sig_hex}"
  }}
}}"#
    ...
    #[test]
    fn load_accepts_pq_signed_exit_descriptor() {
        // A PQ-signed exit descriptor (its ML-KEM key bound under the operational
        // signature) must load rather than be rejected, so the app can consume a
        // PQ-publishing exit. The classical `/v1` verifier alone rejects it (the
        // signature is over the PQ payload, not the `/v1` one), which is exactly
        // the pre-change behavior this guards against.
        let dir = isolated_tempdir();
        let op = SigningKey::from_bytes(&[0x77; 32]);
        let json = pq_signed_exit_file(&op);
        std::fs::write(dir.join(MULTI_HOP_FILENAME), &json).expect("write");

        let cfg = load_from_settings_dir(&dir)
            .expect("PQ-signed exit descriptor must verify")
        ...
        assert!(
            cfg.exit.exit_mlkem768_pubkey.is_some(),
            "the signed ML-KEM key must round-trip into the loaded exit descriptor"
        );
