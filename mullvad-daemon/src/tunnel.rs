...
use talpid_core::tunnel_state_machine::{
    SubscriptionStatus, SubscriptionStatusProvider, TunnelParametersGenerator,
};
...
/// ADR 36: how long a drained exit stays in the local avoid-set. The engine
/// drain policy owns the value (shared with the SDK rotation avoid-set); long
/// enough to bridge a drain-triggered reconnect to the next ambient
/// relay-list refresh, short enough that a recovered exit is offered again
/// without a daemon restart.
pub(crate) const WARREN_DRAINED_EXIT_TTL_SECS: u64 =
    talpid_warren_tunnel::drain_policy::DRAINED_EXIT_AVOID_TTL.as_secs();
...
struct InnerParametersGenerator {
    ...
    /// Client entry-RTT store: smoothed RTTs the tunnel measured toward
    /// entry nodes, keyed by the entry's Ed25519 pubkey. Fed by the
    /// multi-hop supervisor's `on_path_rtt` observer (wired in
    /// `produce_warren_tunnel_params`); snapshotted by the directory
    /// updater as the client-measured half of the shared path-aware
    /// selection. Shared (`Arc`) because the observer outlives a single
    /// param-generation call; process-lifetime, no persistence.
    warren_entry_rtt: Arc<std::sync::Mutex<warren_discovery_core::RttCache>>,
    ...
    /// Live-reconfig channel for the client-side bandwidth ceiling
    /// (`Settings::warren_max_rate_bps`, bits per second, `None` =
    /// unlimited). Same discipline as `nat_pmp_control_tx`: the sender
    /// fans a settings change out to every active tunnel's rate
    /// limiter without a reconnect; the receiver is cloned into
...
impl ParametersGenerator {
    ...
    pub fn new_with_optional_warren(
        ...
        Self(Arc::new(Mutex::new(InnerParametersGenerator {
            ...
            // Initial watch value `None` = uncapped. The daemon
    ...
    /// Snapshot of the client entry-RTT store for the directory
    /// updater's path-aware selection. Empty (neutral: today's exact
    /// weight ordering) until a multi-hop tunnel feeds it.
    pub async fn warren_entry_rtt_snapshot(&self) -> warren_discovery_core::RttCache {
        self.0
            .lock()
            .await
            .warren_entry_rtt
    ...
    /// Sets the user's client-side bandwidth ceiling (bits per second,
    /// `None` = unlimited) and fans it out to every live tunnel over
    /// the internal watch channel, so the cap applies without a
    /// reconnect. Fresh tunnels read the current value at start via
    /// the receiver handed out by `produce_warren_tunnel_params`.
    pub async fn set_warren_max_rate_bps(&self, bps: Option<u64>) {
        let inner = self.0.lock().await;
        // `send` errs iff no receivers exist (no tunnel up), which is
        // fine: the next tunnel start reads the value via `borrow()`.
        let _ = inner.max_rate_control_tx.send(bps);
    }
    ...
    pub async fn produce_warren_tunnel_params(
        ...
        // logged-out wallet the provider yields an empty stack and the
        // supervisor falls back to the v6 wallet-signed path.
        ...
        // it: every v6 packet is silently dropped. Gating here connects
        // such an exit v4-only; the
        ...
        // Feed the client entry-RTT store from the supervisor's lifecycle
        // samples (session publish + close), keyed by the DIALED relay's
        // Ed25519 pubkey: the client-measured half of the shared
        ...
        // Same live wiring for the client-side bandwidth ceiling: the
...
impl SubscriptionStatusProvider for ParametersGenerator {
    /// Signed `GET /v1/subscription`, mapped to the three answers the tunnel
    /// state machine can act on.
    ///
    /// Called only after an exit has refused a session, i.e. while the
    /// blocking state is up, which is why it must go through the daemon's own
    /// API transport: that one resolves the host from the address cache the
    /// firewall permits by IP, so this reaches the API where system DNS
    /// cannot. Anything short of a definite answer is `Unknown`, which leaves
    /// the state machine on its timed fallback rather than on a guess derived
    /// from a network failure.
    ///
    /// See [`subscription_status_from`] for how one answer becomes a verdict.
    fn subscription_status(&self) -> Pin<Box<dyn Future<Output = SubscriptionStatus>>> {
        let this = self.clone();
        Box::pin(async move {
            let (api_url, seed) = {
                let inner = this.0.lock().await;
                (
                    inner.warren_api_url.clone(),
                    inner.warren_identity_seed.clone(),
                )
            };
            let (Some(api_url), Some(seed)) = (api_url, seed) else {
                // No wallet or no API configured: nothing to ask, and nothing
                // to conclude from not asking.
                return SubscriptionStatus::Unknown;
            };
            // One transport for the life of the daemon: an ask lands in the
            // middle of a refusal loop, on a 3 s budget, and a fresh client
            // would pay a cold TLS handshake out of it every time.
            let transport = {
                let mut inner = this.0.lock().await;
                inner
                    .warren_api_transport
                    .get_or_insert_with(crate::warren_api_transport::WarrenApiTransport::new)
                    .clone()
            };
            let client = crate::warren_sdk_client::SharedWarrenApiClient::with_transport(
                api_url, seed, transport,
            );
            let answer = client.get_subscription().await;
            if let Err(error) = &answer {
                // The `Display` of a `ClientError` carries a status code or a
                // transport class, never the body, the URL or the address.
                log::debug!("subscription status: {error}");
            }
            subscription_status_from(answer, warrenguard_config::unix_now())
        })
    }
}

/// How far in the past an expiry must be before it is called expired.
///
/// The API answers `200 {expires_at}` for a subscription that exists, expired
/// or not, so the client's own wall clock is what separates "expired" from
/// "active" here. A clock that runs fast would otherwise tell a paying user
/// their account has run out, which is the failure this whole mechanism
/// exists to remove. Inside the margin the answer is `Unknown`, which costs
/// the genuinely just-expired user the old timed wait and nothing else.
const EXPIRY_SKEW_MARGIN_SECS: u64 = 3600;

/// One `GET /v1/subscription` answer, as a verdict the tunnel state machine
/// can act on.
///
/// Pure, so the mapping is testable without a server: it decides whether a
/// user is told their account has run out.
fn subscription_status_from(
    answer: Result<warren_api::SubscriptionResponse, warren_api::ClientError>,
    now_unix: u64,
) -> SubscriptionStatus {
    match answer {
        Ok(subscription) if subscription.expires_at > now_unix => SubscriptionStatus::Active,
        // Expired, and by more than any ordinary clock drift.
        Ok(subscription)
            if subscription
                .expires_at
                .saturating_add(EXPIRY_SKEW_MARGIN_SECS)
                <= now_unix =>
        {
            SubscriptionStatus::Inactive
        }
        // Expired by less than the margin: our clock could be the wrong one.
        Ok(_) => SubscriptionStatus::Unknown,
        // The API's "this wallet has no subscription" answer, which owes
        // nothing to any clock.
        Err(warren_api::ClientError::ServerStatus { status: 404, .. }) => {
            SubscriptionStatus::Inactive
        }
        // Unreachable, rate-limited, 5xx, malformed: no verdict, so the state
        // machine stays on its timed fallback instead of reading a network
        // failure as an account verdict.
        Err(_) => SubscriptionStatus::Unknown,
    }
}
...
mod subscription_status_tests {
    use super::{EXPIRY_SKEW_MARGIN_SECS, SubscriptionStatus, subscription_status_from};

    const NOW: u64 = 1_800_000_000;

    fn subscription(
        expires_at: u64,
    ) -> Result<warren_api::SubscriptionResponse, warren_api::ClientError> {
        Ok(warren_api::SubscriptionResponse { expires_at })
    }

    fn server_status(
        status: u16,
    ) -> Result<warren_api::SubscriptionResponse, warren_api::ClientError> {
        Err(warren_api::ClientError::ServerStatus {
            status,
            body: String::new(),
        })
    }
    ...
    #[test]
    fn a_subscription_with_time_left_is_active() {
        assert_eq!(
            subscription_status_from(subscription(NOW + 1), NOW),
            SubscriptionStatus::Active,
            "an exit refusing this wallet is behind, and the refusal must be waited out"
        );
    }

    #[test]
    fn a_wallet_the_api_does_not_know_is_inactive() {
        // The measured case: a wallet that never registered. Nothing here
        // depends on a clock, so it is answered at the first refusal.
        assert_eq!(
            subscription_status_from(server_status(404), NOW),
            SubscriptionStatus::Inactive
        );
    }

    #[test]
    fn a_long_expired_subscription_is_inactive() {
        assert_eq!(
            subscription_status_from(subscription(NOW - EXPIRY_SKEW_MARGIN_SECS), NOW),
            SubscriptionStatus::Inactive
        );
    }

    #[test]
    fn an_expiry_within_clock_drift_is_not_called_expired() {
        // The API returns 200 for a subscription row whether or not it has
        // run out, so this comparison is the only thing standing between a
        // fast client clock and telling a paying user their account is over.
        assert_eq!(
            subscription_status_from(subscription(NOW - 1), NOW),
            SubscriptionStatus::Unknown,
            "a barely-past expiry must fall back to the timed policy, never accuse"
        );
    }

    #[test]
    fn an_api_that_cannot_answer_yields_no_verdict() {
        for answer in [
            server_status(500),
            server_status(429),
            Err(warren_api::ClientError::AllHostsBlocked),
        ] {
            assert_eq!(
                subscription_status_from(answer, NOW),
                SubscriptionStatus::Unknown,
                "a network or server failure is not an account verdict"
            );
        }
    }
}
