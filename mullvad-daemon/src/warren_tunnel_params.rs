...
use warrenguard_multihop::{
    ExitDescriptorSigned, RelayDescriptorSigned, relay_descriptor_signing_payload,
};
...
/// and nothing returns (a v4-only exit has no v6 upstream). When
...
/// when `None` no multi-hop session is assembled. The exit
...
///
/// **Failover may only ever NARROW the choice.** When the narrowing
/// leaves nothing (the failed exit is the only one matching a pinned
/// country, or the roster refreshed and no longer knows it), this
/// degrades to [`assemble_for_attempt`] and retries the same exit
/// rather than failing. Failing there turned a transient, self-healing
/// exit refusal into `NoMatchingRelay`, which the state machine treats
/// as a terminal blocking error: measured 2026-08-03 on the beta fleet,
/// a just-subscribed wallet left the whole host with no network until
/// the user disconnected by hand.
///
/// # Errors
///
...
pub fn assemble_failover_for_attempt(
    ...
    let Some(alternative) = alternative else {
        log::info!(
            "Warren: no failover alternative under the current constraints; \
             retrying the same exit (a refusal that clears on its own must not \
             strand the user in a blocking error state)"
        );
        return assemble_for_attempt(
            selector,
            signing_key,
            query,
            retry_attempt,
...
///
/// The user supplied an `(endpoint, Ed25519 pubkey, X25519 multihop
/// pubkey, exit_id, cover_domain?)` out of band (e.g. a self-hosted
/// `warrenguard serve --multihop` node). We dial it as a **1-hop
/// multi-hop circuit** whose entry relay and exit are the same node
/// (`single_node = true`, `relay_id == exit_id`), because the fleet
/// speaks only the multi-hop wire protocol now. No roster lookup, no
/// location query, no failover pool. The TOFU pin is skipped by the
/// caller because typing the keys *is* the pin.
///
/// The client transport re-verifies the RELAY descriptor's operational
/// signature at dial time (`multihop::dial_relay` ->
/// `verify_relay_descriptor`) but never the EXIT descriptor's. A
/// hand-entered node has no operational signer, so we self-sign the relay
/// descriptor with an EPHEMERAL Ed25519 key generated here and carry that
/// key as [`MultiHopConfig::operational_pubkey`]: the client's structural
/// PKI check then passes. This changes no trust. The real anchors remain
/// the hand-entered Ed25519 pin (relay TLS RPK / relay-auth proof) and the
/// X25519 HPKE key (payload seal); the exit signature is left `[0u8; 64]`
/// because the client never verifies it.
///
/// `country_code` is empty and `city` carries the user's cosmetic label.
/// The root `exit_addr`/`exit_id` are populated for firewall/routing
/// shape but ignored on the multi-hop path (the dispatcher derives the
/// exit identity from [`MultiHopConfig::exit`]).
///
/// # Errors
///
/// Returns [`AssembleError::CustomExit`] if the endpoint is not a valid
/// `host:port`, the Ed25519 pubkey is not 64 hex chars or not a valid
/// curve point, the X25519 multihop pubkey is not 64 hex chars, or the
/// exit id is not 32 hex chars.
pub fn assemble_custom(
    ...
    // The Ed25519 key is pinned in-band (RPK / relay-auth proof) but never
    // re-signed client-side, so reject a non-curve-point now: a typo must
    // fail at assembly, not deep in the QUIC handshake.
    VerifyingKey::from_bytes(&pubkey_bytes).map_err(|e| {
        AssembleError::CustomExit(format!("pubkey is not a valid Ed25519 point: {e}"))
    })?;

    let exit_id = ExitId::from_hex(custom.exit_id_hex.trim())
        .map_err(|e| AssembleError::CustomExit(format!("exit_id: {e}")))?;
    let exit_id_bytes = *exit_id.as_bytes();

    let x25519_bytes: [u8; 32] = {
        let raw = hex::decode(custom.x25519_multihop_pubkey_hex.trim())
            .map_err(|e| AssembleError::CustomExit(format!("x25519 multihop pubkey: {e}")))?;
        raw.try_into().map_err(|v: Vec<u8>| {
            AssembleError::CustomExit(format!(
                "x25519 multihop pubkey: expected 32 bytes, got {}",
                v.len()
            ))
        })?
    };

    let cover_domain = custom
        .cover_domain
        .as_deref()
        .map(str::trim)
        .filter(|d| !d.is_empty())
    ...
    if let Some(domain) = &cover_domain {
        // A hand-entered exit carries no signed roster, so it cannot advertise
        // the per-exit carrier bit the roster path threads. Arm the anti-
        // censorship TLS-over-TCP carrier for any custom exit that sets a cover
        // domain: the carrier needs exactly that cover-domain SNI (X.509), and a
        // user who opts a custom exit into X.509 is reaching for the escape hatch
        // that keeps working on a UDP-hostile network. The carrier stays dormant
        // unless the UDP handshake fails, and a custom exit whose server does not
        // run the terminator merely fails one RTT slower on a blocked network, so
        // arming it is the correct default rather than leaving the escape hatch
        // UDP-only.
        exit_addr = exit_addr
            .with_cover_domain(domain.clone())
            .with_tcp_fallback();
    }

    // Ephemeral self-sign of the relay descriptor (see fn doc). Seed from
    // `rand::random()` rather than `SigningKey::generate` to avoid coupling
    // to a specific rand_core version; the key is trust-free and discarded
    // after signing.
    let ephemeral = SigningKey::from_bytes(&rand::random());
    let relay_signature = ephemeral
        .sign(&relay_descriptor_signing_payload(
            &exit_id_bytes,
            &pubkey_bytes,
        ))
        .to_bytes();
    let operational_pubkey = ephemeral.verifying_key();

    let multi_hop = MultiHopConfig {
        ...
        exit: ExitDescriptorSigned {
            exit_id,
            exit_ed25519_pubkey: pubkey_bytes,
            exit_x25519_multihop_pubkey: x25519_bytes,
            endpoint: Some(endpoint),
            cover_domain,
            // Left unsigned: the client never verifies the exit descriptor
            // signature (only the relay one, self-signed above).
            signature: [0u8; 64],
            dns_disabled: false,
            exit_mlkem768_pubkey: None,
        },
        operational_pubkey,
        // Empty geo: a hand-entered node is not in the signed directory, so
        // the caller falls back to the single-hop relay-list lookup for the
        // location label. `city` reuses the cosmetic label below.
        exit_country: String::new(),
        exit_city: custom.label.clone(),
        enable_gso: true,
        use_warren_obfuscation: true,
        // Entry relay and exit are the same node: a 1-hop circuit.
        single_node: true,
    };
    ...
    Ok(WarrenTunnelParameters {
        ...
        n_connections: DEFAULT_N_CONNECTIONS,
        features: DEFAULT_FEATURES,
        // Empty -> client offers the `ALPN_H3` default inside the
        // multi-hop client, which is exactly what a plain `warrenguard
...
mod tests {
    ...
    fn in_tunnel_ipv6_negotiated_only_when_exit_attests_egress() {
        // Blackhole guard: a v4-only exit
    ...
    #[test]
    fn failover_retries_the_same_exit_when_it_is_the_only_candidate() {
        // Measured 2026-08-03 against the beta fleet: an exit refuses a
        // just-subscribed wallet with `not-allowlisted` (transient, it
        // clears at the exit's next allowlist poll). The retry excludes
        // that exit, and with a pinned country holding a single exit the
        // candidate set empties, so the connect died on NoMatchingRelay
        // and the daemon parked in its blocking error state with no
        // retry: the whole host stayed dark until the user disconnected
        // by hand. Failover may only ever NARROW the choice; when
        // narrowing leaves nothing, retrying the same exit is the only
        // outcome that cannot strand the user.
        let list = WarrenRelayList::new(vec![fixture_relay(1, "de"), fixture_relay(2, "fr")]);
        let selector = DaemonWarrenRelaySelector::new(list);
        let query = WarrenRelayQuery::any().with_location(LocationConstraint::Country("de".into()));

        let params = assemble_failover_for_attempt(
            &selector,
            fixture_signing_key(),
            &query,
            1,
        ...
        assert_eq!(
            params.exit_id,
            fixture_relay(1, "de").exit_id(),
            "the only exit matching the pinned country must be re-selected",
        );
    }

    #[test]
    fn failover_retries_the_same_exit_when_the_roster_forgot_it() {
        // Same lockout by another route: the roster refreshed between
        // the failed attempt and the retry, so the remembered pubkey no
        // longer resolves. Failing here would block the connect on a
        // bookkeeping detail the user cannot see or act on.
        let list = WarrenRelayList::new(vec![fixture_relay(3, "se")]);
        let selector = DaemonWarrenRelaySelector::new(list);
        ...
        assert_eq!(
            params.exit_id,
            fixture_relay(3, "se").exit_id(),
            "the live roster entry must be selected",
        );
    }

    #[test]
    fn failover_still_prefers_an_alternative_when_one_exists() {
        // The narrowing itself must survive the fix: with two exits in
        // the pinned country, the failed one is still avoided.
        let list = WarrenRelayList::new(vec![fixture_relay(1, "de"), fixture_relay(2, "de")]);
        let selector = DaemonWarrenRelaySelector::new(list);
        let query = WarrenRelayQuery::any().with_location(LocationConstraint::Country("de".into()));
        ...
        assert_eq!(
            params.exit_id,
            fixture_relay(2, "de").exit_id(),
            "an available alternative must still be preferred over the failed exit",
        );
    }

    #[test]
    fn failover_propagates_the_error_when_the_roster_is_empty() {
        // The degradation must not mask a genuinely unusable roster:
        // with nothing to select, the selector error still surfaces.
        let selector = DaemonWarrenRelaySelector::new(WarrenRelayList::new(Vec::new()));

        let err = assemble_failover_for_attempt(
            &selector,
            fixture_signing_key(),
            &WarrenRelayQuery::any(),
            1,
    ...
    /// A genuine Ed25519 verifying key in hex. `assemble_custom` validates
    /// the pubkey as a curve point, so fixtures cannot use arbitrary bytes.
    fn fixture_pubkey_hex() -> String {
        hex::encode(
    ...
    #[test]
    fn assemble_custom_dials_the_user_supplied_exit_as_one_hop_multihop() {
        // A valid custom exit must produce a 1-hop multi-hop circuit
        // (single_node, relay_id == exit_id) pointed at exactly the entered
        // endpoint + keys, with no ALPN override (so the client speaks the
        ...
        assert!(
            ...
            "an RPK custom exit (no cover domain) cannot run the carrier, so it stays UDP-only"
        );

        let mh = params
            .multi_hop
            .as_ref()
            .expect("custom exit is a 1-hop multi-hop circuit");
        assert!(
            mh.single_node,
            "custom exit collapses entry relay and exit onto one node"
        );
        // relay_id == exit_id: the node routes the datagram to its own local
        // termination.
        assert_eq!(mh.relay.relay_id, [0xdd; 16]);
        assert_eq!(*mh.exit.exit_id.as_bytes(), [0xdd; 16]);
        let entered_pubkey = WarrenPubkey::from_hex(&pubkey_hex).unwrap();
        assert_eq!(mh.relay.relay_ed25519_pubkey, *entered_pubkey.as_bytes());
        assert_eq!(mh.exit.exit_ed25519_pubkey, *entered_pubkey.as_bytes());
        assert_eq!(mh.exit.exit_x25519_multihop_pubkey, [0xcc; 32]);
        assert_eq!(mh.relay.endpoint, "198.51.100.7:443".parse().unwrap());
        assert_eq!(mh.exit.endpoint, "198.51.100.7:443".parse().ok());

        // THE key design decision: the ephemeral self-sign must satisfy the
        // exact re-verification the client transport runs at dial time
        // (`dial_relay` -> `verify_relay_descriptor`).
        warrenguard_multihop::verify_relay_descriptor(&mh.operational_pubkey, &mh.relay)
            .expect("ephemeral self-sign must pass the client relay-descriptor re-verification");
        // The exit descriptor is deliberately left unsigned (never verified
        // client-side).
        assert_eq!(mh.exit.signature, [0u8; 64]);
    }
    ...
    fn assemble_custom_applies_cover_domain_for_x509_mode() {
        ...
        // The cover domain also rides both multi-hop descriptors so the
        // client dials the SNI on the relay and the C2 leg targets it too.
        let mh = params.multi_hop.expect("1-hop multi-hop circuit");
        assert_eq!(mh.relay.cover_domain.as_deref(), Some("cdn.example.com"));
        assert_eq!(mh.exit.cover_domain.as_deref(), Some("cdn.example.com"));
    }

    #[test]
    fn assemble_custom_arms_the_carrier_for_a_cover_domain_exit() {
        // A hand-entered exit has no roster carrier bit, so setting a cover
        // domain is the only signal a user can give that this exit is an
        // anti-censorship escape hatch. assemble_custom must arm the carrier
        // for it, otherwise the escape hatch is UDP-only and useless on the
        // exact UDP-hostile network it exists to serve.
        let custom = WarrenCustomExitSettings {
            cover_domain: Some("cdn.example.com".to_owned()),
            ..fixture_custom("198.51.100.7:443", &fixture_pubkey_hex())
        };
        ...
        assert!(
            params
                .multi_hop
                .expect("1-hop multi-hop circuit")
                .relay
                .tcp_fallback,
            "the relay descriptor must also advertise the carrier so a UDP-blocked \
             client retries the same node over TCP"
        );
    }
    ...
    #[test]
    fn assemble_custom_rejects_malformed_x25519_multihop_pubkey() {
        // The X25519 HPKE key must be exactly 64 hex chars (32 bytes): a
        // short/garbage value is a hard error, not a silent single-hop
        // fallback (the multi-hop payload cannot be sealed without it).
        let custom = WarrenCustomExitSettings {
    ...
    #[test]
    fn assemble_custom_rejects_malformed_exit_id() {
        // The exit id must be exactly 32 hex chars (16 bytes): a wrong-length
