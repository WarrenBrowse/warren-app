...
impl From<Error> for ErrorStateCause {
    fn from(error: Error) -> ErrorStateCause {
        match error {
            ...
            // rejection surfaced as `talpid_warren_tunnel::Error::BackendFatal`.
...
impl Error {
    ...
    /// The user-facing reason of a self-healing Warren exit refusal, `None`
    /// for every other error.
    ///
    /// Lets the connecting state tell the one retriable failure that can
    /// persist forever apart from those that cannot, without sniffing error
    /// strings. It is deliberately NOT wired into
    /// [`From<Error> for ErrorStateCause`]: a refusal is recoverable, so that
