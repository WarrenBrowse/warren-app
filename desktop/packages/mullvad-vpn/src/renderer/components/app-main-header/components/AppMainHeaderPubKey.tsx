...
import { Flex, Icon, LabelTiny, useMainHeaderTone } from '../../../lib/components';
...
const StyledButton = styled.button<{ $dark?: boolean }>(({ $dark }) => ({
  display: 'inline-flex',
  alignItems: 'center',
  gap: '4px',
  padding: 0,
  ...
  color: 'inherit',
  '&:focus-visible': {
    outline: `1px solid ${$dark ? 'var(--color-black-alpha60)' : 'var(--color-white-alpha60)'}`,
    outlineOffset: '2px',
    borderRadius: '4px',
  },
}));
...
const StyledPubKey = styled(LabelTiny)({
  fontFamily:
    ...
    <Flex alignItems="center">
      <StyledButton
        ...
        <Icon
          ...
          size="small"
          color={justCopied ? 'green' : dark ? 'blackAlpha80' : 'whiteAlpha80'}
        />
      </StyledButton>
    </Flex>
  );
