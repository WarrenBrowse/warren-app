import { NatPmpMapping, NatPmpProto, NatPmpRule } from '../../../shared/daemon-rpc-types';
...
/** Human label for a rule protocol. */
export function protocolLabel(protocol: NatPmpProto): string {
  switch (protocol) {
...
/** Whether two rule protocols cover an overlapping transport: `both`
 * shares its external port's TCP and UDP slots, so it collides with any
 * other protocol choice on the same port. */
