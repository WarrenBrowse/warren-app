...
function nextFreePort(rules: NatPmpRule[], protocol: NatPmpProto): number {
  const used = new Set(
    rules.filter((r) => protocolsOverlap(r.protocol, protocol)).map((r) => rulePort(r)),
  );
...
export function PortForwardingAdvanced() {
  ...
  const handleAdd = React.useCallback(() => {
    ...
    const port = nextFreePort(rules, NatPmpProto.both);
    void addRule({
  ...
  const isDuplicate = React.useCallback(
    (port: number, protocol: NatPmpProto, exceptIndex: number) =>
      rules.some(
        (r, i) =>
          i !== exceptIndex && protocolsOverlap(r.protocol, protocol) && rulePort(r) === port,
      ),
    [rules],
  );
      ...
      const protocol =
        e.target.value === 'tcp'
          ...
          : e.target.value === 'both'
            ? NatPmpProto.both
            : NatPmpProto.udp;
      onChangeProtocol(index, protocol);
    },
      ...
      <StyledRow>
        <StyledRowGroup>
          <StyledSelect
            ...
            <option value={NatPmpProto.both}>TCP+UDP</option>
