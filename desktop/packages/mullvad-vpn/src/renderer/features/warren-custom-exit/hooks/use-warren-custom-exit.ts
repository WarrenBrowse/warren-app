...
export function useWarrenCustomExit() {
  ...
  const setX25519MultihopPubkeyHex = React.useCallback(
    (x25519MultihopPubkeyHex: string) => update({ ...warrenCustomExit, x25519MultihopPubkeyHex }),
    [update, warrenCustomExit],
  );

  const setExitIdHex = React.useCallback(
    (exitIdHex: string) => update({ ...warrenCustomExit, exitIdHex }),
    [update, warrenCustomExit],
  );
