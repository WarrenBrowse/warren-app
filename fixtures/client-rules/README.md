...
| `forum_outcomes.json` attach | `client_rules.rs` (`attach_outcome_for_response`, `attach_envelope`, the client-side failures) | `WarrenForumAttachUseCaseTest` (decodes `envelope`, the client-side failures; `terminal_kinds` through `isTerminalAttachOutcome`) | none (the desktop maps the answers in `main/forum-attach.ts`, which has no fixture reader yet) | none |
