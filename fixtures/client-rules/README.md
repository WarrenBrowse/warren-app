...
| `product_env.json` | `warren-product-env/tests/client_rules.rs` (API, update and on-disk anchors), `warren-forum` `client_rules.rs` (connect host, forum origin) | `ProductEnvBuildConfigTest` (`BuildConfig` of the running flavor, the forum host, the connect host) | `product-env.spec.ts` (`product-env.ts`, `tasks/distribution.cjs`) | none yet (no product-env plumbing on iOS) |
