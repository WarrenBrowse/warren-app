...
| `forum_outcomes.json` login completion | none (how the sid reached the app is a platform fact the crate never sees; the crate's half is the `completion` of the login cases) | `ForumLoginCompletionTest` (every case's `envelope` through `parseForumLoginOutcome`, then `forumCompletionPlan`; `code_lifetime_secs` against `FORUM_LOGIN_CODE_LIFETIME_MILLIS`) | `forum-login.spec.ts` (`parseForumLoginCompletion` over every login case, `forumCompletionPlan` over every completion case from the answer's `body`, `code_lifetime_secs` against `FORUM_LOGIN_CODE_LIFETIME_MS`) | `WarrenForumLinkTests` (every case's `envelope` through `WarrenAccountClient.forumLoginOutcome`, then `WarrenForumLinks.completionPlan`; `code_lifetime_secs` against `codeLifetime`) |
...
| `forum_activity.json` | none (the digest's signature and freshness are `warren-forum`'s; how one user's badge is read out of it is per platform) | `ForumActivityFixtureTest` (`unreadForSlot`, `unreadLabel`, `forumHeaderButton`, `showsForumActivity`, `forumActivityWording`, and every monitor storm through `ForumActivityMonitor`) | `forum-activity-client-rules.spec.ts` (the same six, and the storms through `main/forum-activity-monitor.ts`) | `WarrenForumActivityTests` (`WarrenForumActivity` and the storms through `WarrenForumActivityMonitor`) |
| `scenery_layout.json` | none (the connect backdrop is drawn per platform, so no crate computes this) | `SceneryLayoutFixtureTest` (`SceneryLayout.placement` over every case, the pan and slide clamps, the band bounds) | pending (the desktop window ratio is the canvas ratio, so every clamp lands at its no-op value; a reader would pin exactly that) | pending |
...
(`ios-checks.yml`, on push and PR) and read the fixture directory from
...
- `attach.cases[]`: same shape for `POST /v1/forum/attach-logs`; `expect` is
  `{kind, reason?}` with `kind` in `attach._kinds`. The two successes
  (`attached` for a topic, `received` for a pre-topic session) are one kind:
  the client has nothing else to do either way.
- `attach.max_log_gz_bytes`: the largest gzipped report a client sends, the
  shared crate's `MAX_LOG_GZ_BYTES` (the broker's base64 field cap in bytes);
  every platform's size gate is held to it.
- `attach.terminal_kinds`: the outcomes after which Approve is disarmed. A
  terminal outcome other than `expired` leaves the session pending on the
  broker, so leaving the prompt still cancels it.
- `attach.client_side_failures.cases[]`: as for login.
