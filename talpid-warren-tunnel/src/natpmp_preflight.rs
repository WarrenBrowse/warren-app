...
/// never block it. Empty when port-forwarding is disabled. A dual-proto
/// rule expands into one reservation per leg on the SAME external port;
/// `reserve_all`'s all-or-release semantics keep the pair atomic on the
/// candidate too.
#[must_use]
pub fn pinned_rules(cfg: &crate::NatPmpConfig) -> Vec<PinnedRule> {
    ...
    cfg.effective_rules()
        ...
        .flat_map(|r| {
            r.protocol.legs().iter().map(|&leg| PinnedRule {
                proto: leg,
...
pub async fn reserve_one<T: PreflightTransport>(
    ...
    for _ in 0..attempts {
        ...
        if internal_port != rule.internal_port || proto != rule.proto {
            // A response for a different rule. Matching the proto too is
            // load-bearing for dual-proto pins: both legs share the same
            // internal port, so the port alone cannot tell them apart.
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn pinned_rules_expands_a_dual_proto_pin_into_both_legs() {
        // A pinned TCP+UDP rule must reserve BOTH legs on the candidate
        // exit, on the same external port, or the post-swap pair could
        // find half its port taken.
        let mut cfg = NatPmpConfig::default_enabled();
        cfg.rules = vec![NatPmpRule {
            protocol: crate::NatPmpProto::Both,
            suggested_external_port: 27015,
            internal_port: 27015,
            sticky_suggestion: false,
        }];
        let pins = pinned_rules(&cfg);
        assert_eq!(pins.len(), 2, "one reservation per leg: {pins:?}");
        assert!(pins.iter().any(|p| p.proto == MapProto::Udp));
        assert!(pins.iter().any(|p| p.proto == MapProto::Tcp));
        assert!(pins.iter().all(|p| p.external_port == 27015));
    }
