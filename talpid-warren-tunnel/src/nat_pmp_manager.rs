...
use warrenguard_natpmp_client::{
    CredentialProvider, RefreshLoopConfig, RefreshLoopHandle, spawn_refresh_loop_with,
};
...
pub struct NatPmpManager {
    ...
    /// The entitlement this rule presents, asked once per refresh cycle so a
...
impl NatPmpManager {
    ...
    pub fn start_from_addr(
        ...
        Self::start_with_credential(runtime, server, config, observer, bind_addr, None)
    }

    /// [`start_from_addr`] presenting `credential` with every request, for a
    /// rule that holds a port entitlement (warren-core doc 99).
    #[must_use = "the returned manager owns the spawned tasks; drop discards control"]
    ...
    fn spawn_refresh_pair(
        ...
        let refresh_handle = spawn_refresh_loop_with(
            RefreshLoopConfig {
                server,
                protos: config.protocol,
                internal_port: config.internal_port,
                suggested_external_port: config.suggested_external_port,
