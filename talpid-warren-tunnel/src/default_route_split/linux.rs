...
//! cleanup, and the ownership-scoped crash recovery) lives once in warrenguard-route-split
...
//! TUN. We inject that here via warrenguard-route-split's `split_tunnel_fwmark` parameter,
...
//! The v6 guard ([`DefaultRouteSplitV6Guard`]) wraps warrenguard-route-split the same way
