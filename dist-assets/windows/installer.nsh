# Macros that are invoked by the Warren installer and uninstaller.
# (Adapted from Mullvad upstream; the Windows service identifier is `warrenvpn`,
# matching the daemon's `SERVICE_NAME` constant in mullvad-daemon/src/lib.rs.)
...
# ExtractWintun
#
# Extract Wintun installer into $PLUGINSDIR
#
!macro ExtractWintun
...
!macro InstallService
	...
	${If} $0 != 0
		StrCpy $R0 "Failed to install Warren VPN service"
		log::LogWithDetails $R0 $1
	...
	nsExec::ExecToStack '"$SYSDIR\sc.exe" start warrenvpn'
	...
	${AndIf} $0 != ${SERVICE_START_PENDING}
		${If} $0 == ${ERROR_SERVICE_DEPENDENCY_DELETED}
			StrCpy $R0 'Failed to start Warren VPN service: The firewall service "Base Filtering Engine" is missing or unavailable.'
		${Else}
			StrCpy $R0 "Failed to start Warren VPN service: error $0"
		${EndIf}
		log::LogWithDetails $R0 $1
		Goto InstallService_return
	${EndIf}
...
!macro InstallTrayIcon
	...
	${If} $0 != ${MULLVAD_SUCCESS}
		log::LogWithDetails "Failed to install Warren VPN tray icon" $1
...
!macro customInstall
	...
	# The split-tunnel driver is ONE machine-wide resource (service
	# `mullvad-split-tunnel`, device `\\.\MULLVADSPLITTUNNEL`), not a
	# per-install one. A non-production build has no claim on it: resetting it
	# would pull the driver out from under a production install, and the
	# attempt fails outright (0x1) while that install holds it, aborting an
	# install that never needed the reset in the first place.
	!ifndef WARREN_NON_PROD_INSTALL
		${RemoveSplitTunnelDriver}
...
!macro StopAndDeleteService
	...
	nsExec::ExecToStack '"$SYSDIR\sc.exe" query warrenvpn'
	...
	log::Log "Stopping Warren VPN service"

	nsExec::ExecToStack '"$SYSDIR\net.exe" stop warrenvpn'
	...
	log::Log "Removing Warren VPN service"
	nsExec::ExecToStack '"$SYSDIR\sc.exe" delete warrenvpn'
	...
	${If} $0 != ${ERROR_SERVICE_DOES_NOT_EXIST}
		log::Log "Attempting to forcibly kill Warren VPN service"

		nsExec::ExecToStack '"$SYSDIR\taskkill.exe" /f /fi "SERVICES eq warrenvpn"'
		Pop $0
		Pop $1
		...
		StrCpy $R0 "Failed to kill Warren VPN service"
		log::Log $R0
		Goto StopAndDeleteService_return_only
	${EndIf}
