...
use mullvad_api::{
    ETag, RelayListProxy, StatusCode,
    rest::{self, Response},
};
...
impl SwiftMullvadApiResponse {
    ...
    /// Response whose body was rebuilt Rust-side after signature
    /// verification (the signed `/v1/exits` directory projected to the
    /// relay-list wire shape), rather than passed through verbatim.
    pub fn verified_body(body: Vec<u8>, etag: Option<ETag>) -> Result<Self, rest::Error> {
        let body_size = body.len();
        let body = body.into_boxed_slice();

        let etag = match etag {
            Some(etag) => CString::new(etag.0)
                .map_err(|_| rest::Error::InvalidHeaderError)?
                .into_raw(),
            None => ptr::null_mut(),
        };
    ...
    /// Failure surfaced when a fetched body does not pass Warren
    /// signature verification. `success = false` makes the Swift side
    /// treat it as a request failure, so it keeps its cached relay list
    /// (same keep-last-good behavior as the desktop updater).
    pub fn warren_verify_error(description: &str) -> Self {
        let error_description = CString::new(description)
