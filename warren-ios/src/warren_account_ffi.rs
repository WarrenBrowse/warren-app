...
/// warren-api base URL of the compiled product environment (no trailing
/// slash). Kept in lockstep with Android's `PRODUCT_API_URL` (`warren-jni`)
/// and the daemon's `warren_product_config::WARREN_API_URL` so a signed
/// request a phone makes is verifiable by the same backend the other
/// platforms talk to.
