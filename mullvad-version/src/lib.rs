...
static VERSION_REGEX: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?x)                                     # enable insignificant whitespace mode
                (?<major>0|[1-9]\d{0,3})\.         # major component (0-9999; semver major or legacy year)
...
mod tests {
    ...
    #[test]
    fn test_parses_a_zero_major_version() {
        // The major component was year-shaped upstream, so it never started at
        // zero. Warren's beta line restarts at 0.0.1, and rejecting it made the
        // Windows version.h generator panic mid-build with an error that named
        // the resource compiler rather than the version.
        let parsed = Version::from_str("0.0.1").unwrap();
        assert_eq!(parsed.major, 0);
        assert_eq!(parsed.minor, 0);
        assert_eq!(parsed.patch, Some(1));
        assert_eq!(parsed.to_string(), "0.0.1");

        assert_eq!(Version::from_str("0.1.0").unwrap().to_string(), "0.1.0");
    }
